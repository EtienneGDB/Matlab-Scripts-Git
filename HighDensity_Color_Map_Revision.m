...
for iGroup = 1%:2
    ...
    for iM = 1:length(Muscles)
%         for iC = 7:-1:2
%             meas.(Muscles{iM})(:,iC) = meas.(Muscles{iM})(:,iC)-meas.(Muscles{iM})(:,2);
%         end
...
%         matrice = rot90(rot90(M6_MedFreq));
%         save('matrice_colormapI6.mat','matrice')

        figure
        subplot(1,6,1);
        imagesc(rot90(rot90(M1_MedFreq)))
        set(gca,'ColorScale','log')
        view([-180 90])
        
%         figure
        subplot(1,6,2); imagesc(M2_MedFreq)
        hold on
...
%         figure
        subplot(1,6,3); imagesc(M3_MedFreq)
        hold on
        for iM = 1:length(Muscles)-5
            [row,col] = find(schem == iM);
...
%         figure
        subplot(1,6,4); imagesc(M4_MedFreq)
        hold on
        for iM = 1:length(Muscles)-5
            [row,col] = find(schem == iM);
...
%         figure
        subplot(1,6,5); imagesc(M5_MedFreq)
        hold on
        for iM = 1:length(Muscles)-5
            [row,col] = find(schem == iM);
        ...
        subplot(1,6,6);imagesc(M6_MedFreq)
        hold on
        for iM = 1:length(Muscles)-5
            [row,col] = find(schem == iM);
...
%         imagesc(M6_MedFreq-M1_MedFreq,[-20 0])
        imagesc(watershed(rot90(rot90(M6_MedFreq))-rot90(rot90(M1_MedFreq))))
%         bw = rot90(rot90(M6_MedFreq))-rot90(rot90(M1_MedFreq));
        
        [X] = imread('C:\Users\p1098713\Documents\2.Post-Doc\Articles\Article Piano Fatigue\Figures\testwatershed.tif');
        X = (X(:,:,1));
        X = imadjust(X);
        imshow(X)
