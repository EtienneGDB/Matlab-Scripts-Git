...
impVIP_Ha<-readMat("J:\\Piano_Fatigue\\Data_Exported\\VIP_CIRCOS_Review_Ha.mat")
impVIP_Li<-readMat("J:\\Piano_Fatigue\\Data_Exported\\VIP_CIRCOS_Review_Li.mat")
impVarNames_Ha<-readMat("J:\\Piano_Fatigue\\Data_Exported\\VarNames_CIRCOS_Review_Ha.mat")
impVarNames_Li<-readMat("J:\\Piano_Fatigue\\Data_Exported\\VarNames_CIRCOS_Review_Li.mat")

#Membres<-data.frame('Pelvis','Trunk','Head','Shoulder','Arm','Forearm','Hand')

Task = 'Li'
if(Task=='Ha'){j=1; Color_bar='blue'; impVarNames=impVarNames_Ha; L=length(impVIP_Ha[[1]]); impVIP=impVIP_Ha$VIPSCORE.CIRCOS.REVIEW
}else{j=2; Color_bar='red'; impVarNames=impVarNames_Li; L=length(impVIP_Li[[1]]); impVIP=impVIP_Li$VIPSCORE.CIRCOS.REVIEW}

VarNames=matrix('Na',nrow=L,ncol=3)
for(i in 1:length(impVarNames[[1]])){
  VarNames[[i]]<-impVarNames[[1]][[i]][[1]]
}
VarNames=matrix(VarNames,nrow=L,ncol=3)

VIPSCORE<-data.frame(id=c(1:L),X=VarNames,Y=impVIP,Task=rep(Task,L))
