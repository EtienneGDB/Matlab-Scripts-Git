...
for iSubjects = 1:length(Subjects)
    ...
    if iSubjects == any([2:16 18:19 21:length(Subjects)])
...
%         plot(EMG_envelop)
        ...
        for iNormalization = 1:length(Muscles)
            Normalization (:,iNormalization) = EMG_envelop(:,iNormalization)./EMG_max(1,iNormalization)*100;
        end
%         figure ; plot(Normalization(:,4))

        % Dection of activity
        Seg = ActivityDetection2(EMGBL, {'deltant' 'biceps' 'uptrap'}, 1000, 1, 2000, Muscles, FileNames{2,iFiles}(1));
        pause
