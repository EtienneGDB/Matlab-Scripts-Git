...
if WATCH
    ...
    Watch_allData = [];
    Time_allData = [];
    watchFolder = dir(['F:\Projet RPQ\ExportedData\Watch\RPQProject\P', num2str(Participant) ,'\Recordsets\*H*']);
    for iwatchFolder = 1:length(watchFolder)
        for iFileNames = 1:length(FileNames)
            Folder = fullfile([saveDir, '\Watch\RPQProject\P', num2str(Participant), '\Recordsets\', watchFolder(iwatchFolder).name]);
...
%             end
    ...
    cd([saveDir, '\Home\Watch'])
    save(['P', num2str(Participant), '_Watch_Home', '.mat'], 'Watch_allData')
    save(['P', num2str(Participant), '_TimeWatch_Home', '.mat'], 'Time_allData')
